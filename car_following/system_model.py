...
def angle_change_positive(x):
    _, _, _, _, steering_angle_ego = x
    
    return -steering_angle_ego

def angle_change_negative(x):
    _, _, _, _, _, steering_angle_ego = x
...
def get_random_target_state(x,max_change_x,min_change_x,max_change_y,min_change_y,min_change_angle,max_change_angle):
    ...
    straight_lane = torch.rand(1).item() < 0.5
    ...
    new_angle = angle + min(delta_angle_min,delta_angle_max) if straight_lane else angle
