...
from system_model import angle_change_negative_lower_bound,angle_change_positive_upper_bound,\
                            ...
                            car_following_estimation,car_following_lower_bound,car_following_upper_bound,positive_velocity

# Training setup
state_dim, hidden_dim, output_dim = 5, 512, 2  # 2D state (position, velocity), control is 1D
n_steps = 3
...
for init_condition in range(30):
    ...
    for t in range(num_predictions):
        ...
        for step in range(n_steps):
            if torch.isnan(u[step]).any():
                print("controls are nan:",u[step])    
            ...
            if torch.isnan(u[step,0]).any():
                print("modified acceleration positive/negative is nan:",u[step,0])    
            ...
            velocity_positive_constraint_rhs = positive_velocity(x.squeeze(0))

            velocity_positive_constraint = -u[step,0]+velocity_positive_constraint_rhs
            ...
            acceleration_normalized_constraints_rhs = torch.vstack([acceleration_upper_bound_rhs,acceleration_lower_bound_constraints_rhs,
                                                                    acceleration_difference_lower_bound_rhs,acceleration_difference_upper_bound_rhs,velocity_positive_constraint_rhs])
            acceleration_normalized_constraints = torch.vstack([acceleration_upper_bound_constraint,acceleration_lower_bound_constraint,
                                                                    acceleration_difference_lower_bound_constraint,acceleration_difference_upper_bound_constraint,velocity_positive_constraint])
            acceleration_normalized_constraints = torch.relu(acceleration_normalized_constraints)
            # print(acceleration_normalized_constraints)
            acceleration_violated_constraints = 1/2+1/2*torch.tanh(steepness*(acceleration_normalized_constraints.sum(dim=0))-tanh_displacement)
            acceleration_normalized_constraints = acceleration_normalized_constraints -torch.max(acceleration_normalized_constraints,dim=0).values
            ...
            u[step,0] = u[step,0] + acceleration_violated_constraints.unsqueeze(0)*((acceleration_weights*acceleration_normalized_constraints_rhs).sum(dim=0).unsqueeze(0)-u[step,0])
            if torch.isnan(u[step,0]).any():
                print("modified acceleration smoothness is nan:",u[step,0])    
            # print("bounded acceleration is:",u[step,0])
            # if t%10==0:
                # print("state and target heading are:",x,x_target)
                # input('hipi')
            ...
            if torch.isnan(u[step,1]).any():
                print("modified steering angle is to direction nan:",u[step,1])    
            ...
            u[step,1] = u[step,1] + steering_angle_rate_violated_constraints.unsqueeze(0)*\
                ((steering_angle_rate_weights*steering_angle_rate_normalized_constraints_rhs).sum(dim=0).unsqueeze(0)-u[step,1])
            if torch.isnan(u[step,1]).any():
                print("modified steering angle is to smoothnesss nan:",u[step,1])    
            ...
            if torch.isnan(new_x).any() or torch.isnan(u[step]).any():
                print('NAN in new_x or u[step] at time step',step)
                print(x)
                print(x_target)
                print(new_x,u[step])
                input('yipi')
                break
            x=new_x
            list_u.append(u[step])
            list_x.append(x-x_target)       

    prev = torch.stack(list_x).squeeze(1).detach().numpy()
    prev_u = torch.stack(list_u).detach().numpy()
    # if epoch %  10 ==0:
    final_distance = x
    data['sample {} x'.format(init_condition)] = prev[:num_predictions*n_steps,0]
    data['sample {} y'.format(init_condition)] = prev[:num_predictions*n_steps,1]
    data['sample {} v'.format(init_condition)] = prev[:num_predictions*n_steps,2]
    data['sample {} heading'.format(init_condition)] = prev[:num_predictions*n_steps,3]
    data['sample {} steering_angle'.format(init_condition)] = prev[:num_predictions*n_steps,4]
    data['sample {} acceleration'.format(init_condition)] = prev_u[:num_predictions*n_steps,0]
    data['sample {} steering_angle_rate'.format(init_condition)] = prev_u[:num_predictions*n_steps,1]
